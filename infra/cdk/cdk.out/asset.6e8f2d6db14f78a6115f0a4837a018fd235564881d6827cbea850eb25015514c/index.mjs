...
export const handler = async (event) => {
  ...
  try {
    ...
    // 1) Token lesen (Email & nette Fehler)
    ...
    // 3) Optional: Welcome-Mail
    if (email) { try { await sendWelcomeEmail(email); } catch (e) { console.warn('welcome mail failed', e); } }

    // 4) Redirect zur Ergebnis-Seite
