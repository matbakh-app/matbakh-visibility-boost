import { isSuppressed } from '../common/suppressionGuard.mjs';
...
function matchOrigin(origin, pattern) {
  ...
  try {
    ...
    // Wildcard-Pattern (nur Subdomain-Wildcards wie https://*.vercel.app)
...
function originAllowed(origin) {
  for (const a of ALLOWED) {
    if (matchOrigin(origin, a)) return true;
  }
  return false;
}
...
async function sendDoiEmail(to, token, event) {
  ...
  // Confirm-Link bauen: bevorzugt API-GW-Domain/Stage, sonst CONFIRM_BASE oder Fallback
...
export const handler = async (event) => {
  ...
  try {
    ...
    // --------- NEU: Suppression-Check vor Persistenz & Versand ---------
