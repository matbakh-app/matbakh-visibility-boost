...
-- FINAL VERSION - Berücksichtigt existierende Spalten
-- =========================================================
...
-- 1) PROFILES - Existierende Tabelle erweitern statt neu erstellen
-- Erst prüfen welche Spalten existieren
do $$
declare
  has_name_col boolean;
  has_role_col boolean;
  has_display_name_col boolean;
...
begin
  ...
  ) into has_display_name_col;

  -- Prüfe ob name nullable ist
  ...
  raise notice 'Profile table analysis: name=%, role=%, display_name=%, name_nullable=%', 
    has_name_col, has_role_col, has_display_name_col, name_nullable;

  -- Wenn name NOT NULL ist, setze Defaults für NULL-Werte
  ...
  -- Role-Spalte hinzufügen falls nicht vorhanden
  if not has_role_col then
    alter table public.profiles add column role text not null default 'owner';
  else
    -- Existierende role-Spalte aktualisieren
    update public.profiles set role = coalesce(role, 'owner') where role is null;
...
-- Fallback: Tabelle erstellen falls sie gar nicht existiert
...
begin
  ...
  -- Prüfe name-Spalte für Super-Admin Insert
  select exists(
    select 1 from information_schema.columns 
    where table_schema='public' and table_name='profiles' and column_name='name'
  ) into has_name_col;
  ...
  -- Bestehende Policies defensiv entfernen
  ...
  -- Trigger-Funktion: legt Profile + private_profiles beim Signup an
  if has_name_col and not name_nullable then
    -- Wenn name NOT NULL ist, setze einen Default-Wert
    execute format('
    ...
    -- Standard-Version ohne name-Feld
    execute format('
      create or replace function public.handle_new_user()
      returns trigger
      security definer
  ...
  -- Super-Admins zuweisen (mit korrekten Spalten)
  if has_name_col and not name_nullable then
    -- Mit name-Feld
    execute format('
      insert into public.profiles (%I, role, name)
      select id, ''super_admin'', coalesce(email, ''Super Admin'')
      from auth.users
      where email in (''info@matbakh.app'',''mail@matbakh.app'',''matbakhapp2025@gmail.com'')
      on conflict (%I) do update set 
        role = excluded.role,
        name = coalesce(excluded.name, public.profiles.name, ''Super Admin'');
    ', pkcol, pkcol);
  else
    -- Ohne name-Feld
    execute format('
  ...
  -- private_profiles für Super-Admins sicherstellen
  execute '
    insert into public.private_profiles (user_id, email, full_name)
    select 
      id, 
      email, 
      coalesce(raw_user_meta_data->>''name'', email, ''Super Admin'')
    from auth.users
    where email in (''info@matbakh.app'',''mail@matbakh.app'',''matbakhapp2025@gmail.com'')
    on conflict (user_id) do update
    set 
...
select 
  ...
  u.email, 
  p.role,
  case when exists(select 1 from information_schema.columns where table_schema='public' and table_name='profiles' and column_name='name') 
       then p.name else 'N/A' end as name_field
from auth.users u 
