-- ============================================================================
-- PHASE 1: Critical Security Hardening - RLS Policies and Audit Logging (Fixed)
-- ============================================================================
...
DROP POLICY IF EXISTS "Admins can view audit logs" ON service_audit_log;
...
DROP POLICY IF EXISTS "Allow public read access for active packages" ON service_packages;
DROP POLICY IF EXISTS "Admins only can insert packages" ON service_packages;
DROP POLICY IF EXISTS "Admins only can update packages" ON service_packages;
DROP POLICY IF EXISTS "Admins only can delete packages" ON service_packages;
...
DROP POLICY IF EXISTS "Allow public read access for service prices" ON service_prices;
DROP POLICY IF EXISTS "Admins only can insert prices" ON service_prices;
DROP POLICY IF EXISTS "Admins only can update prices" ON service_prices;
DROP POLICY IF EXISTS "Admins only can delete prices" ON service_prices;
...
-- 5. Add security constraints for partner_kpis validation (if not exists)
-- ============================================================================

-- Add check constraints for KPI value ranges (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_annual_revenue_range') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_seating_capacity_range') THEN
    ALTER TABLE partner_kpis 
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_food_cost_ratio_range') THEN
    ALTER TABLE partner_kpis 
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_labor_cost_ratio_range') THEN
    ALTER TABLE partner_kpis 
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_employee_count_range') THEN
    ALTER TABLE partner_kpis 
...
-- Add constraint to validate additional_kpis structure (if not exists)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_additional_kpis_structure') THEN
    ALTER TABLE partner_kpis 
