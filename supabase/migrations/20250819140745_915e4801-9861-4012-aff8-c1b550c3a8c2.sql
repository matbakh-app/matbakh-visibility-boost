-- Task 5: Complete RLS and schema alignment
...
-- 3) fb_* tables - Partner-bound RLS
...
CREATE POLICY IF NOT EXISTS partner_config_read ON public.fb_conversions_config FOR SELECT
USING (partner_id IN (SELECT id FROM public.business_partners WHERE user_id = auth.uid()));

CREATE POLICY IF NOT EXISTS partner_config_write ON public.fb_conversions_config FOR INSERT
WITH CHECK (partner_id IN (SELECT id FROM public.business_partners WHERE user_id = auth.uid()));

CREATE POLICY IF NOT EXISTS partner_config_update ON public.fb_conversions_config FOR UPDATE
USING (partner_id IN (SELECT id FROM public.business_partners WHERE user_id = auth.uid()))
WITH CHECK (partner_id IN (SELECT id FROM public.business_partners WHERE user_id = auth.uid()));

CREATE POLICY IF NOT EXISTS partner_logs_read ON public.fb_conversion_logs FOR SELECT
USING (partner_id IN (SELECT id FROM public.business_partners WHERE user_id = auth.uid()));
