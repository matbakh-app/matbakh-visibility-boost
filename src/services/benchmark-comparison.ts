/**
 * Industry Benchmark Comparison Service - AWS RDS Native
...
export class BenchmarkComparisonService {
  ...
  async getIndustryBenchmarks(
    ...
    try {
      ...
      const result = await this.rdsClient.executeQuery(query, [
        industryId,
        regionId,
        scoreType,
      ]);
  ...
  private calculateComparison(
    ...
    if (currentScore <= benchmark.percentile_25) {
      ...
      percentileRank =
        25 +
        25 *
          ((currentScore - benchmark.percentile_25) /
            (benchmark.percentile_50 - benchmark.percentile_25));
    } else if (currentScore <= benchmark.percentile_75) {
      percentileRank =
        50 +
        25 *
          ((currentScore - benchmark.percentile_50) /
            (benchmark.percentile_75 - benchmark.percentile_50));
    } else if (currentScore <= benchmark.percentile_90) {
      percentileRank =
        75 +
        15 *
          ((currentScore - benchmark.percentile_75) /
      ...
      percentileRank =
        ...
        10 *
          Math.min(
            1,
            (currentScore - benchmark.percentile_90) /
              (benchmark.percentile_90 * 0.1)
          );
    }
    ...
    const improvementPotential = Math.max(
      0,
      benchmark.percentile_75 - currentScore
    );

    // Generate position descriptions
    const industryPosition = this.getPositionDescription(
      percentileRank,
      "industry"
    );
    const regionalPosition = this.getPositionDescription(
      percentileRank,
      "region"
    );
  ...
  private generateRecommendations(
    ...
    switch (category) {
      case "well_below":
        recommendations.push(
          "Sofortige Maßnahmen zur Verbesserung der Sichtbarkeit erforderlich"
        );
        recommendations.push(
          "Fokus auf Grundlagen: Google My Business Optimierung"
        );
        recommendations.push(
          "Regelmäßige Content-Erstellung und Social Media Aktivität"
        );
        break;
      ...
      case "below":
        ...
        recommendations.push(
          "Verstärkte Kundeninteraktion und Review-Management"
        );
      ...
      case "average":
        recommendations.push(
          "Kontinuierliche Optimierung für Wettbewerbsvorteile"
        );
        recommendations.push(
          "Spezialisierung auf besonders erfolgreiche Kanäle"
        );
      ...
      case "well_above":
        recommendations.push(
          "Marktführerposition nutzen für weitere Expansion"
        );
        recommendations.push("Thought Leadership und Branding stärken");
        recommendations.push(
          "Mentoring anderer Standorte (bei Franchise-Betrieben)"
        );
        break;
    }
    ...
    if (currentScore < targetScore) {
      ...
      recommendations.push(
        `Ziel: Verbesserung um ${
          Math.round(gap * 100) / 100
        } Punkte für Top 25% Position`
      );
    }
  ...
  async getMultiRegionBenchmarks(
    ...
    try {
      ...
      const data = result.records.map(
        (record) => this.rdsClient.mapRecord(record) as ScoreBenchmark
      );
      ...
      data.forEach((benchmark: ScoreBenchmark) => {
        if (
          !benchmarksByRegion[benchmark.region_id] ||
          benchmark.last_updated >
            benchmarksByRegion[benchmark.region_id].last_updated
        ) {
  ...
  async updateBenchmark(
    benchmark: Omit<ScoreBenchmark, "id" | "last_updated">
  ): Promise<boolean> {
    try {
