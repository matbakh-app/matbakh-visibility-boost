...
// AWS-native AI service management
...
class AIServiceManager {
  ...
  async refreshPortfolio(): Promise<void> {
    try {
      // Get feature flags for AI services from AWS
      const response = await fetch(
        "/api/feature-flags?filter=ai,bedrock,gemini",
        {
      ...
      const bedrockRollout = this.getFlagValue(
        flags,
      ...
      const healthScore =
        services.length > 0
          ? Math.round(
              services.reduce((sum, s) => sum + s.healthScore, 0) /
                services.length
            )
          : 0;
   ...
   */
  private async buildBedrockService(
    ...
    const capabilities: AICapability[] = [
      {
        ...
        description:
          "AI-powered business visibility analysis with SWOT, Porter's Five Forces",
        category: "analysis",
        enabled: true,
      },
      {
        ...
        description:
          "Automatic user persona identification and adaptive responses",
        category: "insights",
        enabled: true,
      },
      {
    ...
    const capabilities: AICapability[] = [
      {
        ...
        name: "Image Analysis",
        description:
          "AI-powered image content analysis and optimization suggestions",
        category: "analysis",
        enabled: false,
        beta: true,
      },
  ...
  getPersonaCapabilities(
    personaType: PersonaAIConfig["personaType"]
  ): AICapability[] {
    if (!this.portfolio) return [];

    const personaConfig = this.getPersonaConfig(personaType);
    const allCapabilities = this.portfolio.services.flatMap(
      (s) => s.capabilities
    );
