...
const AngeboteDE: React.FC = () => {
    ...
    return (
      <>
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
          {packages.map((pkg) => (
            <PricingCard key={pkg.id} package={pkg} lang="de" />
          ))}
        </div>
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <TrustElements lang="de" />
      <ProcessOverview lang="de" />
      {packages && packages.length > 0 && (
        <PackageComparison packages={packages} lang="de" />
      )}
      <PackageFAQ lang="de" />
