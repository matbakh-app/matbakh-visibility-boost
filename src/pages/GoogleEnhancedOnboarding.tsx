...
import { BarChart3, Building, Camera, Edit3, Globe, Loader2, MapPin, Save, Star } from 'lucide-react';
...
export default function GoogleEnhancedOnboarding() {
  ...
  const loadGMBData = async () => {
    try {
      // Get current user session from AWS Cognito
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const user = await Auth.currentAuthenticatedUser();
      console.log('[DEBUG] User:', user?.attributes?.email);
      ...
      // First create business partner via AWS API
      const partnerResponse = await fetch('/api/business-partners', {
        method: 'POST',
        headers: {
      ...
      if (!partnerResponse.ok) {
        const partnerError = await partnerResponse.text();
        console.error('[DEBUG] Partner creation error:', partnerError);
        alert('Fehler: Business Partner konnte nicht erstellt werden: ' + partnerError);
        return;
      }
      ...
      const profileResponse = await fetch('/api/business-profiles', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${(await Auth.currentSession()).getIdToken().getJwtToken()}`
      ...
      if (!profileResponse.ok) {
        ...
        alert('Fehler: Profil konnte nicht gespeichert werden: ' + error);
