...
export class LegacyComponentDetector {
  ...
  private static classifySafeToArchive(
    ...
    if (isCriticalPath) {
      return {
        safeToArchive: false,
        reason: 'Critical system component - requires manual review',
        riskLevel: 'critical'
      };
    }
    ...
    if (activeBackendDeps.length > 0 && !activeBackendDeps.every(dep => dep.migrationPath)) {
      return {
        safeToArchive: false,
        reason: 'Has active backend dependencies without migration path',
        riskLevel: 'high'
      };
    }
    ...
    if (origin === 'supabase' || origin === 'lovable') {
      ...
      if (hasKiroAlternative) {
        return {
          safeToArchive: true,
          reason: 'Legacy component with Kiro alternative available',
          riskLevel: 'low'
        };
      }
    }
