...
/**
 * Brand-specific formatting utilities
 */
...
export const getBrandUrl = (language: Language): string => {
  return language === 'de' ? 'https://sichtbarkeitsanalyse.de' : 'https://visibilitycheck.com';
};

export const formatPhoneNumber = (phone: string, language: Language): string => {
  // German format: +49 30 12345678 -> +49 30 12 34 56 78
  // English format: +49 30 12345678 -> +49 30 1234 5678
...
export const formatAddress = (address: string, city: string, country: string, language: Language): string => {
  if (language === 'de') {
 ...
 */
export const formatReservationTime = (time: string, language: Language): string => {
...
export const formatCapacity = (current: number, max: number, language: Language): string => {
  const percentage = Math.round((current / max) * 100);
  
  if (language === 'de') {
    return `${current} von ${max} (${percentage}%)`;
  } else {
    return `${current} of ${max} (${percentage}%)`;
  }
};

export const formatWaitTime = (minutes: number, language: Language): string => {
  if (minutes < 60) {
