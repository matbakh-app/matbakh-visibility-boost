 ...
 *
 * Provides a clean abstraction over data access using AWS services.
 */
...
function notImplemented(feature: string): never {
  ...
  throw new Error(
    `${feature} endpoint not yet migrated to AWS. Add to migration backlog.`
  );
}
...
export const dataApi = {
  ...
  kpis: {
    summary(restaurantId: string) {
      return http<{ score: number; trends: any[] }>(
        `/api/kpi/summary?rid=${encodeURIComponent(restaurantId)}`
      );
    },
    history(restaurantId: string, period: string = "30d") {
      return http<{ data: any[] }>(
        `/api/kpi/history?rid=${encodeURIComponent(
          restaurantId
        )}&period=${period}`
      );
    },
    ...
    diversified() {
      return notImplemented("KPI diversified analysis");
    },
    enhanced() {
      return notImplemented("Enhanced KPI analysis");
    },
  },
  ...
  auth: {
    profile() {
      return http<{ id: string; email: string; role: string }>(
        "/api/auth/profile"
      );
    },
  ...
  business: {
    contact(restaurantId: string) {
      return http<{ name: string; address: any; phone: string }>(
        `/api/business/contact?rid=${encodeURIComponent(restaurantId)}`
      );
    },
    categories(restaurantId: string) {
      return http<{ categories: string[] }>(
        `/api/business/categories?rid=${encodeURIComponent(restaurantId)}`
      );
    },
    // Stubs for complex migrations
    smartCategories() {
      return notImplemented("Smart category selection");
    },
    diversifiedCategories() {
      return notImplemented("Diversified category selection");
    },
  },
  ...
  vc: {
    start(data: any) {
      return http<{ id: string; status: string }>("/api/vc/start", {
        method: "POST",
        body: data,
      });
    },
    result(vcId: string) {
      return http<{ score: number; analysis: any }>(
        `/api/vc/result/${encodeURIComponent(vcId)}`
      );
    },
  ...
  google: {
    connection(restaurantId: string) {
      return http<{ connected: boolean; profile: any }>(
        `/api/google/connection?rid=${encodeURIComponent(restaurantId)}`
      );
    },
    // Stub - complex OAuth flow
    connect() {
      return notImplemented("Google OAuth connection");
    },
  },
  ...
  ai: {
    recommendations(restaurantId: string, type: string = "general") {
      return http<{ recommendations: any[] }>(
        `/api/ai/recommendations?rid=${encodeURIComponent(
          restaurantId
        )}&type=${type}`
      );
    },
  ...
  features: {
    access(userId: string, feature: string) {
      return http<{ hasAccess: boolean; plan: string }>(
        `/api/features/access?uid=${encodeURIComponent(
          userId
        )}&feature=${feature}`
      );
    },
  ...
  partner: {
    profile(partnerId: string) {
      return http<{ id: string; name: string; tier: string }>(
        `/api/partner/profile/${encodeURIComponent(partnerId)}`
      );
    },
  },
  ...
  realtime: {
    connection() {
      return notImplemented(
        "Realtime connection - consider WebSocket/SSE alternative"
      );
    },
  },
};
