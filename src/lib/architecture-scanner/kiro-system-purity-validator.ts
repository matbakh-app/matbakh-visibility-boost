...
const DEFAULT_PATTERNS = [
  ...
  '!src/**/__tests__/**',
  '!src/**/?(*.)+(spec|test).*',
  // große/versteckte Ordner ausschließen
  '!**/node_modules/**',
  '!**/dist/**',
  '!**/build/**',
  '!**/.next/**',
...
// Files finden – **ohne** Symlink-Folgen, begrenzte Tiefe, mit Abort-Signal
export async function findProjectFiles(options?: {
  ...
  const files = await fg(patterns, {
    ...
    stat: false,
    maxDepth: 12,         // schützt vor Deep-Trees
...
export class KiroSystemPurityValidator {
  ...
  async scan(opts?: {
    ...
    try {
      ...
      if (opts?.verbose) console.log(`🔎 Purity: scanning ${files.length} files…`);

      // Lesen/Analysieren mit Concurrency-Limit (z.B. 24)
      await mapLimit(files, 24, async (file) => {
        if (ac.signal.aborted) throw ac.signal.reason;
        // Nur lesen, wenn wirklich nötig:
  ...
  private async validateTestFrameworks(): Promise<TestFrameworkValidationResult> {
    console.log('🔍 Validating test frameworks...');
  ...
  private generateRecommendations(violations: PurityViolation[]): string[] {
    ...
    if (violations.length === 0) {
      recommendations.push('🎉 System is pure! All components are Kiro-based.');
    ...
    if (highViolations.length > 0) {
      recommendations.push(`⚠️ Address ${highViolations.length} high-priority violations`);
...
System Status: ${result.isPure ? '✅ PURE' : '❌ IMPURE'}

## Executive Summary

${result.isPure 
  ? '🎉 Congratulations! Your system has achieved Kiro purity certification.'
  : '⚠️ Your system contains legacy components that need migration to achieve purity.'
}
...
- Kiro Configured: ${result.validations.testFrameworks.isKiroConfigured ? '✅' : '❌'}
...
${result.certification 
  ? `✅ **CERTIFIED**: ${result.certification.certificationLevel.toUpperCase()}
- Issued: ${result.certification.issuedAt}
- Valid Until: ${result.certification.validUntil}
- API Purity: ${result.certification.criteria.apiPurity ? '✅' : '❌'}
- Test Purity: ${result.certification.criteria.testPurity ? '✅' : '❌'}
- Component Purity: ${result.certification.criteria.componentPurity ? '✅' : '❌'}
- Configuration Purity: ${result.certification.criteria.configurationPurity ? '✅' : '❌'}`
  : '❌ **NOT CERTIFIED**: System does not meet purity requirements'
}
