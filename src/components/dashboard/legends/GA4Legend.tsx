...
const GA4Legend: React.FC<GA4LegendProps> = ({ lang }) => {
  ...
  console.log('[GA4Legend] lang:', lang, 'ready:', ready, 'sessions:', t('charts.legend.sessions'));
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-4 mt-4" key={lang}>
      <span className="flex items-center gap-2">
        ...
        <span className="text-sm">{t('charts.legend.sessions', 'Sitzungen')}</span>
      </span>
      <span className="flex items-center gap-2">
        <span className="inline-block w-3 h-3 rounded bg-[#10b981]" />
        <span className="text-sm">{t('charts.legend.users', 'Nutzer')}</span>
      </span>
    </div>
  );
};
