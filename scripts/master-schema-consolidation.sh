...
echo "🎯 MASTER SCHEMA CONSOLIDATION - Task 12.1 FINAL"
echo "================================================="

echo ""
echo "✅ Ziel: Konfliktfreie service_packages Struktur"
echo "🧹 Bereinigung aller redundanten Migrationen"
echo "🔄 Konsolidierung mit AWS RDS-kompatiblem Schema"

echo ""
echo "📋 Step 1: Backup erstellen"
BACKUP_TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
echo "Backup timestamp: $BACKUP_TIMESTAMP"

mkdir -p ./backups
supabase db dump > "./backups/full_backup_before_service_package_fix_${BACKUP_TIMESTAMP}.sql"
echo "✅ Backup erstellt: ./backups/full_backup_before_service_package_fix_${BACKUP_TIMESTAMP}.sql"
...
echo "Erstelle konsolidierte service_packages Migration..."# Er
stelle neue Migration mit Zeitstempel
...
-- 6. Daten-Migration von Legacy-Tabelle (falls vorhanden)
INSERT INTO service_packages (
  id, code, name, slug, default_name, description, base_price, original_price, 
  is_active, is_recurring, is_recommended, interval_months, min_duration_months
)
SELECT 
  id,
  COALESCE(code, slug, 'PKG_' || id::text) as code,
  COALESCE(name, default_name, 'Package') as name,
  slug,
  COALESCE(default_name, name, 'Package ' || id::text) as default_name,
  description,
  base_price,
  original_price,
  COALESCE(is_active, true) as is_active,
  COALESCE(is_recurring, period = 'monthly', false) as is_recurring,
  COALESCE(is_recommended, false) as is_recommended,
  CASE 
    WHEN period = 'monthly' THEN 1
    WHEN interval_months IS NOT NULL THEN interval_months
    ELSE NULL 
...
-- 7. Standard Service Packages einfügen (falls keine Daten vorhanden)
...
-- 8. Entsprechende Preise einfügen
...
CREATE TABLE IF NOT EXISTS google_job_queue (
  ...
  status text DEFAULT 'pending' CHECK (status IN ('pending', 'in_progress', 'error', 'done')),
  error_message text,
  created_at timestamp with time zone DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_google_job_queue_run_at_status ON google_job_queue (run_at, status);
CREATE INDEX IF NOT EXISTS idx_google_job_queue_partner_id ON google_job_queue (partner_id);
...
-- 10. Updated-at Trigger für service_packages
...
echo "✅ Migration erstellt: $MIGRATION_FILE"ec
ho ""
echo "📋 Step 5: Migration anwenden"
...
if command -v supabase &> /dev/null; then
    ...
    if [ $? -eq 0 ]; then
        echo ""
        echo "🎉 MASTER SCHEMA CONSOLIDATION ERFOLGREICH!"
        echo ""
        ...
        echo "- Backup: ✅ Erstellt vor allen Änderungen"
        echo ""
        echo "🔍 Verifikation:"
        ...
        echo ""
        echo "📋 Step 6: Frontend Types aktualisieren"
        echo "Generiere neue TypeScript Types..."
        if command -v supabase &> /dev/null; then
            supabase gen types typescript --local > src/types/supabase.ts 2>/dev/null || echo "Types generation skipped (optional)"
        fi
        ...
        echo "Backup: ./backups/full_backup_before_service_package_fix_${BACKUP_TIMESTAMP}.sql"
        exit 1
    fi
else
