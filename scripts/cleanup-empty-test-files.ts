...
interface EmptyTestFile {
  path: string;
  reason: string;
  action: "skip" | "add-placeholder" | "rename";
}

function findEmptyTestFiles(): EmptyTestFile[] {
  const testFiles = glob.sync("src/**/*.test.{ts,tsx}", {
    ignore: ["src/archive/**", "src/__mocks__/**"],
  });

  const emptyFiles: EmptyTestFile[] = [];
...
function addPlaceholderTest(filePath: string): void {
  const content = readFileSync(filePath, "utf-8");
...
function skipEmptyFile(filePath: string): void {
...
export { addPlaceholderTest, findEmptyTestFiles, skipEmptyFile };
