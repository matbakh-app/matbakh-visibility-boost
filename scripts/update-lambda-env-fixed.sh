...
# Fixed approach to update Lambda environment variables
...
update_lambda() {
    ...
    # Update the DATABASE_URL in the JSON and wrap in Variables object
    jq --arg db_url "$NEW_DATABASE_URL" '{Variables: (. + {"DATABASE_URL": $db_url})}' "$TEMP_FILE" > "${TEMP_FILE}.new"
...
for FUNCTION_NAME in "matbakh-create-tables" "matbakh-get-presigned-url" "matbakh-s3-upload-processor" "matbakh-fix-tables" "matbakh-db-test" "MatbakhVcStack-VcConfirmFnBC142FAD-E7kmx3KDgRRX" "MatbakhVcStack-VcStartFnC5BAD875-Lukpaun5TO53"; do
    echo "📋 $FUNCTION_NAME:"
    aws lambda get-function-configuration \
        --region $REGION \
        --function-name "$FUNCTION_NAME" \
        --query 'Environment.Variables.DATABASE_URL' \
        --output text 2>/dev/null || echo "   No DATABASE_URL found"
done